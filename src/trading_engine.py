...
class TradingEngine:
    def __init__(self, config: dict, broker, db_connection_string: str, 
        ...
        # Database is required for safe trading
        try:
            self.db = DatabaseManager(db_connection_string)
            logger.info("Connected to database for trade logging")
        except Exception as e:
            logger.error(f"Database connection failed: {e}")
            logger.error("CRITICAL: Cannot trade without database - shutting down for safety")
            raise RuntimeError("Database connection required for trading")
