...
Simple SPY scalping with standard profit targets (0.10-0.16%)
"""
...
class ST0CKAStrategy(TradingStrategy):
    ...
    - Target: 0.13% profit per trade (standard scalping target)
    ...
    def __init__(self, mode: str = "simple"):
        ...
        # Strategy parameters - Updated for standard scalping targets
        self.profit_target_pct = 0.0013  # 0.13% profit target (middle of 0.10-0.16% range)
        self.min_profit_target = 0.65    # Minimum $0.65 profit per share
        self.max_profit_target = 1.50    # Maximum $1.50 profit per share
        self.position_size = 1           # Always 1 share (can be increased for larger accounts)
    ...
    def check_exit_conditions(self, position: Position, market_data: Dict[str, Any]) -> Optional[str]:
        ...
        Exit conditions:
        1. Dynamic profit target reached (0.13% of entry price)
    ...
    def get_exit_order_params(self, position: Position, exit_reason: str) -> Dict[str, Any]:
        ...
        if exit_reason == "profit_target":
            # Calculate the exact target price
            ...
            # Time exit or other reasons - use market order
            return {
                'symbol': 'SPY',
                'side': 'sell',
                'order_type': 'market',
    ...
    def _calculate_profit_target(self, entry_price: float) -> float:
        """
        Calculate dynamic profit target based on entry price
        Uses 0.13% of entry price, bounded by min/max targets
        """
        ...
        # Log the target for transparency
        self.logger.debug(f"Profit target for entry at ${entry_price:.2f}: ${target:.2f} ({target/entry_price*100:.3f}%)")
